...
const NavigationMenu = React.forwardRef(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Root
    ref={ref}
    className={cn(
      "relative z-10 flex max-w-max flex-1 items-center justify-center p-[13px]",
      className
    )}
    {...props}>
    {children}
...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-10 w-max items-center justify-center rounded-3xl bg-white px-4 py-2 text-sm font-medium transition-colors hover:bg-gray-200 hover:text-slate-900 focus:bg-gray-200 focus:text-slate-900 disabled:pointer-events-none disabled:opacity-50 dark:bg-slate-950 dark:hover:bg-gray-800 dark:hover:text-white dark:focus:bg-gray-800 dark:focus:text-white"
)
...
const NavigationMenuTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Trigger
    ...
    {children}{""}
    <ChevronDown
      className="relative top-[1px] ml-1 h-5 w-5 transition duration-200 group-data-[state=open]:rotate-180 text-accent"
      aria-hidden="true" />
  </NavigationMenuPrimitive.Trigger>
))
